...
import { qualifyStep1Schema, type QualifyStep1Data } from '@/lib/qualify-validations';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Select } from '@/components/ui/select';

interface Props {
  data: Partial<QualifyStep1Data>;
  onUpdate: (data: Partial<QualifyStep1Data>) => void;
...
export function QualifyStep1_Basics({ data, onUpdate, onValidChange }: Props) {
  const { register, handleSubmit, watch, formState: { errors, isValid }, trigger, setValue } = useForm<QualifyStep1Data>({
    resolver: zodResolver(qualifyStep1Schema),
    defaultValues: {
  ...
  return (
    <form onSubmit={handleSubmit(() => {})} className="space-y-5">
      <div>
        <h2 className="text-2xl font-bold text-white font-display">Let's start with the basics</h2>
        <p className="text-gray-400 mt-1">Takes about 3 minutes. We'll build your ROI estimate as you go.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <Select id="annualRevenue" placeholder="Select range..." options={revenueOptions} {...register('annualRevenue')} />
          {errors.annualRevenue && <p className="text-red-400 text-xs mt-1">{errors.annualRevenue.message}</p>}
        </div>
        <div>
          <Label htmlFor="employeeCount">Team Size (optional)</Label>
          <Select id="employeeCount" placeholder="Select range..." options={employeeOptions} {...register('employeeCount')} />
        </div>
      </div>

      <div>
        <Label htmlFor="industry">Industry *</Label>
        <Select id="industry" placeholder="Select industry..." options={industryOptions} {...register('industry')} />
