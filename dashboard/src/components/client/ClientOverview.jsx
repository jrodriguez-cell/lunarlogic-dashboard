...
export default function ClientOverview({ data, currentDSO, dsoChange, onNavigate }) {
  ...
  const collectionRate = data.collectionEfficiency;

  // Automation activity (derived from payment data)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
          ...
          <Tile
            ...
            value={fmtM(totalNext30)}
            sub={`${next30.length} invoice${next30.length !== 1 ? 's' : ''} · on track`}
            color="var(--teal)"
            onClick={() => onNavigate('cash')}
            hint="click to see cash forecast"
          />
          <Tile
            label="Overdue Now"
            value={fmtM(totalOverdue)}
            sub={`${overdue.length} invoice${overdue.length !== 1 ? 's' : ''} need attention`}
            color={totalOverdue > 0 ? 'var(--red)' : 'var(--green)'}
            onClick={() => onNavigate('action')}
            hint="click to see action plan"
          />
        </div>
      </div>
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: '16px 20px' }}>
        <SectionLabel style={{ marginBottom: 10 }}>AR health breakdown</SectionLabel>
        <ARHealthBar invoices={data.invoices} />
        <div style={{ display: 'flex', gap: 20, marginTop: 12, flexWrap: 'wrap' }}>
          {[
            { color: '#22c55e', label: 'Current — paid on time' },
            { color: '#f59e0b', label: '1–30 days overdue' },
            { color: '#f97316', label: '31–60 days overdue' },
            { color: '#ef4444', label: '60+ days overdue' },
          ].map(l => (
      ...
      {/* Two column: what LunarLogic is doing + customer risk */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>

        {/* What LunarLogic is doing */}
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: '16px 20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 12 }}>
            ...
            <ActivityRow icon="↑" label="Collection rate" value={`${collectionRate}%`} color="var(--teal)" />
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: '16px 20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 12 }}>
            {data.paymentBehavior.map(pb => (
              <div key={pb.customer} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ fontSize: 10, color: 'var(--muted)', marginTop: 12, paddingTop: 8, borderTop: '1px solid var(--border)' }}>
            Risk based on historical payment patterns. LunarLogic sends reminders automatically based on each customer's behavior.
          </div>
        </div>
      </div>
...
function SectionLabel({ children, style = {} }) {
  return (
    <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.09em', marginBottom: 10, ...style }}>{children}</div>
  );
}
...
function Tile({ label, value, sub, color, onClick, hint }) {
  return (
    <div
      onClick={onClick}
      style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: '16px 18px', cursor: onClick ? 'pointer' : 'default', transition: 'background 0.12s' }}
    ...
    >
      ...
      {hint && onClick && <div style={{ fontSize: 9, color: 'var(--muted)', marginTop: 6 }}>{hint}</div>}
    </div>
  );
}
    ...
    <div>
      <div style={{ display: 'flex', height: 20, borderRadius: 6, overflow: 'hidden', gap: 2 }}>
        {buckets.filter(b => b.pct > 0).map((b, i) => (
          <div key={i} style={{ width: `${b.pct}%`, background: b.color, opacity: 0.85, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {b.pct > 10 && <span style={{ fontSize: 9, fontWeight: 700, color: '#fff' }}>{Math.round(b.pct)}%</span>}
          </div>
        ))}
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
        {buckets.map((b, i) => (
          <div key={i} style={{ fontSize: 10, color: 'var(--muted)' }}>{b.pct > 2 ? fmtM(b.amt) : ''}</div>
        ))}
      </div>
    </div>
  );
